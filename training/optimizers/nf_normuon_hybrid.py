...
class NFNorMuonHybrid(torch.optim.Optimizer):
    """NF-NorMuon Hybrid: SF-NorMuon for 2D params + True NF-AdamW (4-bit Triton) for 1D params."""
        ...
        for group in self.param_groups:
            if group['train_mode']:
                ...
                for p in group['params']:
                    state = self.state[p]
                    # 2D Params (ScheduleFree+ uses 'x')
                    if 'x' in state:
                        p.detach().copy_(state['x'])
                    # 1D Params (Standard ScheduleFree implicitly uses p to track x_t)
                    elif 'z' in state:
    ...
    def train(self):
        for group in self.param_groups:
            if not group['train_mode']:
                ...
                for p in group['params']:
                    state = self.state[p]
                    # 2D Params (ScheduleFree+ uses 'y')
                    if 'y' in state:
                        p.detach().copy_(state['y'])
                    # 1D Params (Standard ScheduleFree)
                    elif 'z' in state:
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                # Only 2D parameters participate in the AdamC Polyak global learning rate
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if p.ndim == 2 and group.get('use_aurora', True):
                    ...
                    # 1D parameters use True Triton NF-AdamW (Schedule-Free 4-bit)
                    step_sf_adamw_4bit(p, grad, state["z"], state, group_lr, beta1, decay, ckp1)
