...
TransformerBlock and SLMModel with pure BF16 execution.
"""
...
class SLMModel(nn.Module):
    ...
    def forward(self, input_ids, freqs_cis=None, cos_cache=None, sin_cache=None, use_mtp: bool = True):
            ...
            if self.training:
                def custom_forward(x_in, deltas_in):
                    ...
                    return layer(x_in, d_copy, freqs_cis=freqs_cis,
                                 cos_cache=cos_cache, sin_cache=sin_cache)
                x, deltas = checkpoint(custom_forward, x, list(deltas), use_reentrant=False)  # type: ignore
            else:
                x, deltas = layer(x, deltas, freqs_cis=freqs_cis,
                                  cos_cache=cos_cache, sin_cache=sin_cache)

        x = self.norm(x)
        logits_list = self.mtp(x, use_mtp=use_mtp)
        return logits_list
