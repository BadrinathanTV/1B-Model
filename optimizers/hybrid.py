    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                if p.grad is None:
                    continue
                if group.get("use_riemannian", True) and p.ndim == 2:
                    self._step_aurora(p, group)
                else:
                    self._step_adamw_4bit(p, group)
        return loss

    def _step_aurora(self, p, group):
        ...
        step_fn(p.data, p.grad.data, state["momentum_buffer"],
                eta=group["lr"], weight_decay=group["weight_decay"],
                mu=group["momentum"])

    def _step_adamw_4bit(self, p, group):
